...
sealed trait HttpEntity extends japi.HttpEntity {
   ...
   */
  def transformDataBytes(transformer: () ⇒ ByteStringTransformer): HttpEntity
...
sealed trait RequestEntity extends HttpEntity with japi.RequestEntity with ResponseEntity {
  ...
  override def transformDataBytes(transformer: () ⇒ ByteStringTransformer): RequestEntity
}
...
sealed trait ResponseEntity extends HttpEntity with japi.ResponseEntity {
  ...
  override def transformDataBytes(transformer: () ⇒ ByteStringTransformer): ResponseEntity
}
...
sealed trait UniversalEntity extends japi.UniversalEntity with MessageEntity with BodyPartEntity {
   ...
   * ``newContentLength``.
   */
  def transformDataBytes(newContentLength: Long, transformer: () ⇒ ByteStringTransformer): UniversalEntity
}
...
object HttpEntity {
  ...
  final case class Strict(contentType: ContentType, data: ByteString)
    ...
    override def transformDataBytes(transformer: () ⇒ ByteStringTransformer): MessageEntity = {
      try {
        val t = transformer()
        val newData = ByteStringTransformer.join(t.onNext(data), t.onComplete())
        copy(data = newData)
      } catch {
        case NonFatal(ex) ⇒
    ...
    override def transformDataBytes(newContentLength: Long, transformer: () ⇒ ByteStringTransformer): UniversalEntity = {
      val t = transformer()
      val newData = ByteStringTransformer.join(t.onNext(data), t.onComplete())
  ...
  final case class Default(contentType: ContentType,
    ...
    override def transformDataBytes(transformer: () ⇒ ByteStringTransformer): Chunked = {
      val chunks = data.transform("transformDataBytes-Default", () ⇒ ByteStringTransformerOp(transformer())).
        map(Chunk(_): ChunkStreamPart)

      HttpEntity.Chunked(contentType, chunks)
    }
    override def transformDataBytes(newContentLength: Long, transformer: () ⇒ ByteStringTransformer): UniversalEntity =
      Default(contentType, newContentLength, data.transform("transformDataBytes-with-new-length-Default",
        () ⇒ ByteStringTransformerOp(transformer())))
  ...
  final case class CloseDelimited(contentType: ContentType, data: Source[ByteString])
    ...
    override def transformDataBytes(transformer: () ⇒ ByteStringTransformer): CloseDelimited =
      HttpEntity.CloseDelimited(contentType,
        data.transform("transformDataBytes-CloseDelimited", () ⇒ ByteStringTransformerOp(transformer())))
  ...
  final case class IndefiniteLength(contentType: ContentType, data: Source[ByteString])
    ...
    override def transformDataBytes(transformer: () ⇒ ByteStringTransformer): IndefiniteLength =
      HttpEntity.IndefiniteLength(contentType,
        data.transform("transformDataBytes-IndefiniteLength", () ⇒ ByteStringTransformerOp(transformer())))
  ...
  final case class Chunked(contentType: ContentType, chunks: Source[ChunkStreamPart])
    ...
    override def transformDataBytes(transformer: () ⇒ ByteStringTransformer): Chunked = {
      val newChunks =
        chunks.transform("transformDataBytes-Chunked", () ⇒ new DeterministicOp[ChunkStreamPart, ChunkStreamPart] {
          val byteTransformer = transformer()
          var lastChunk: ChunkStreamPart = null
          var completed = false

          override def onPush(element: ChunkStreamPart, ctxt: Context[ChunkStreamPart]): Directive = {
            val transformedChunk = element match {
              case Chunk(data, ext) ⇒
                val bytes = ByteStringTransformer.noElementAsEmpty(byteTransformer.onNext(data))
                Chunk(bytes, ext)
              case l: LastChunk ⇒
                val lastBytes = ByteStringTransformer.noElementAsEmpty(byteTransformer.onComplete())
                lastChunk = Chunk(lastBytes)
                l
            }

            completed = byteTransformer.isComplete

            if (completed && (lastChunk == null))
              ctxt.pushAndFinish(transformedChunk)
            else
              ctxt.push(transformedChunk)
          }

          override def onPull(ctxt: Context[ChunkStreamPart]): Directive = {
            val done = isFinishing || completed
            if (done && lastChunk == null) {
              val lastBytes = ByteStringTransformer.noElementAsEmpty(byteTransformer.onComplete())
              ctxt.pushAndFinish(Chunk(lastBytes))
            } else if (done)
              ctxt.pushAndFinish(lastChunk)
            else
              ctxt.pull()
          }

          override def onUpstreamFinish(ctxt: Context[ChunkStreamPart]): Directive =
            ctxt.absorbTermination()

          override def onFailure(cause: Throwable, ctxt: Context[ChunkStreamPart]): Directive =
            ctxt.fail(byteTransformer.onError(cause))
