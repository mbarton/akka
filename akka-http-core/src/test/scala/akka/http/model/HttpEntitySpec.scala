...
class HttpEntitySpec extends FreeSpec with MustMatchers with BeforeAndAfterAll {
  ...
  def duplicateBytesTransformer(): ByteStringTransformer =
    new ByteStringTransformer {
      override def onNext(bs: ByteString): ByteString = doubleChars(bs)
      override def onComplete(): ByteString = trailer
    }
