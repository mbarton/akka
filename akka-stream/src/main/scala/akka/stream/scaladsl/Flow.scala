...
trait FlowOps[+Out] {
  ...
  def dropWithin(d: FiniteDuration): Repr[Out] =
    timerTransform("dropWithin", () ⇒ new TimerTransformer[Out, Out] {
      ...
      def onTimer(timerKey: Any) = {
        delegate = identityTransformer.asInstanceOf[TransformerLike[Out, Out]]
        Nil
      }
    })
  ...
  def takeWithin(d: FiniteDuration): Repr[Out] =
    timerTransform("takeWithin", () ⇒ new TimerTransformer[Out, Out] {
      ...
      var delegate: TransformerLike[Out, Out] = identityTransformer.asInstanceOf[TransformerLike[Out, Out]]

      def onNext(in: Out) = delegate.onNext(in)
      override def isComplete = delegate.isComplete
      def onTimer(timerKey: Any) = {
        delegate = takeCompletedTransformer.asInstanceOf[TransformerLike[Out, Out]]
        Nil
      }
    })
  ...
  // FIXME docs
  def transform[T, PushD <: Directive, PullD <: Directive](
...
private[scaladsl] object FlowOps {
  ...
  private val takeCompletedTransformer: TransformerLike[Any, Any] = new TransformerLike[Any, Any] {
    override def onNext(elem: Any) = Nil
    override def isComplete = true
  }

  private val identityTransformer: TransformerLike[Any, Any] = new TransformerLike[Any, Any] {
